 ...
 */
	...
	class Vky_MobileNumberLogin_Customer_AccountController extends Mage_Customer_AccountController{
		...
		public function createPostAction()
		{
			...
			try {
				...
				if (empty($errors)) {
					...
					if(!empty($postMobile)){
						$mobileNumberLogin = Mage::getModel('mobilenumberlogin/mobilenumberlogin');
						
						if($mobileNumberLogin->isMobileExist($postMobile)){
		...
		public function loginPostAction()
		{
			...
			if ($this->getRequest()->isPost()) {
				...
				$customAddressData = $CustomAddress->getData();
				if(!empty($customAddressData)){
			...
			if ($this->getRequest()->isPost()) {
				...
				try {
					...
					if(!empty($postMobile)){
						...
						if($mobileNumberLogin->isMobileExist($postMobile,$currentCustomerId)){
							Mage::getSingleton('core/session')->addError('Mobile number is already in use.');
